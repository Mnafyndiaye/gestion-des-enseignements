...
def read():
    ...
    sql=f"SELECT * FROM remarque ORDER BY `idRemarque` DESC"
    cursor.execute(sql)
...
def renderTreeVIew(data):
    ...
    global treeview
    cols=("idRemarque","idEtudiant","idEnseignant","ContenuRe","marque")
    treeview=ttk.Treeview(treeFrame,show="headings",style="mystyle.Treeview",yscrollcommand=treeScroll.set,columns=cols)
    treeview.heading("idRemarque",text="idRemarque",anchor="w")
    treeview.heading("idEnseignant",text="idEnseignant",anchor="w")
    treeview.heading("idEtudiant",text="idEtudiant",anchor="w")
    treeview.heading("idEnseignant",text="idEnseignant",anchor="w")
    treeview.heading("ContenuRe",text="ContenuRe",anchor="w")
    treeview.heading("marque",text="marque",anchor="w")
    

    treeview.column("idRemarque",width=75)
    treeview.column("idEnseignant",width=75)
    treeview.column("idEtudiant",width=90)
    treeview.column("idEnseignant",width=108)
    treeview.column("ContenuRe",width=108)
    treeview.column("marque",width=108)
...
def deleteStudent():
    ...
    if decision != "yes":
        ...
        try:
            conn = connection()
            cursor = conn.cursor()
            cursor.execute(f"DELETE FROM remarque WHERE idRemarque='{str(deleteData)}'")
            conn.commit()
            conn.close()
        except:
...
def renderAddWindow():
    ...
    addCanvas.create_text(49.0, 10.0, anchor="nw", text="Ajouter remarque", fill="#FFFFFF", font=("Inter SemiBold", 24 * -1))
...
def renderEditWindow(selectedStudData):
    ...
    conn.close()
    editWindow = Tk()
    editWindow.title('Gestion des enseignements')
...
mainCanvas.create_text(73.0,15.0,anchor="nw",text="Gestion des Remarques",fill="#FFFFFF",font=("Inter SemiBold", 36 * -1))

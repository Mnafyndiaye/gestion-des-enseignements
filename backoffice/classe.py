...
def read():
    ...
    sql=f"SELECT * FROM classe ORDER BY `idClasse` DESC"
    cursor.execute(sql)
...
def renderTreeVIew(data):
    ...
    cols=("idClasse","nomClasse","anneeScolaire")
    treeview=ttk.Treeview(treeFrame,show="headings",style="mystyle.Treeview",yscrollcommand=treeScroll.set,columns=cols)
    treeview.heading("idClasse",text="idClasse",anchor="w")
    treeview.heading("nomClasse",text="nomClasse",anchor="w")
    treeview.heading("anneeScolaire",text="anneeScolaire",anchor="w")
    

    treeview.column("idClasse",width=75)
    treeview.column("nomClasse",width=90)
    treeview.column("anneeScolaire",width=108)
...
def deleteStudent():
    ...
    if decision != "yes":
        ...
        try:
            conn = connection()
            cursor = conn.cursor()
            cursor.execute(f"DELETE FROM classe WHERE idClasse='{str(deleteData)}'")
            conn.commit()
            conn.close()
        except:
...
def renderAddWindow():
    ...
    def addStudent():
        nomClasse = str(addFnameEntry.get())
        anneeScolaire = str(addLnameEntry.get())
        
        if (nomClasse == "" or nomClasse == " ") or (anneeScolaire == "" or anneeScolaire == " "):
            ...
            try:
                ...
                conn=connection()
                cursor=conn.cursor()
                cursor.execute(f"INSERT INTO classe (nomClasse,anneeScolaire) VALUES ('{nomClasse}','{anneeScolaire}') ")
                conn.commit()
                conn.close()
            except:
    ...
    addCanvas.create_text(49.0, 10.0, anchor="nw", text="Ajout de classe", fill="#FFFFFF", font=("Inter SemiBold", 24 * -1))
    ...
    addCanvas.create_text(326.0, 147.0, anchor="nw", text="Nom de classe", fill="#000000", font=("Inter", 11 * -1))
    ...
    addCanvas.create_text(326.0,211.0,anchor="nw",text="Annee Scolaire",fill="#000000",font=("Inter", 11 * -1))
...
mainCanvas.create_text(73.0,15.0,anchor="nw",text="Gestion des classes",fill="#FFFFFF",font=("Inter SemiBold", 36 * -1))

...
def read():
    ...
    sql=f"select * from rp ORDER BY `idRp` DESC"
    cursor.execute(sql)
...
def deleteStudent():
    ...
    if decision != "yes":
        ...
        try:
            conn = connection()
            cursor = conn.cursor()
            cursor.execute(f"DELETE FROM rp WHERE idRP='{str(deleteData)}'")
            conn.commit()
            conn.close()
        except:
...
def renderAddWindow():
    ...
    def addStudent():
        ...
        if (idEnseignant == "" or idEnseignant == " ") or (prenom == "" or prenom == " ") or (nom == "" or nom == " "):
            ...
            try:
                ...
                conn=connection()
                cursor=conn.cursor()
                cursor.execute(f"INSERT INTO rp (nom,prenom,idEnseignant) VALUES ('{nom}','{prenom}','{idEnseignant}')")
                conn.commit()
                conn.close()
            except:
    ...
    addCanvas.create_text(325.0, 85.0, anchor="nw", text="id Enseignant", fill="#000000", font=("Inter", 11 * -1))
...
mainCanvas.create_text(73.0,15.0,anchor="nw",text="Responsable pedagogique",fill="#FFFFFF",font=("Inter SemiBold", 36 * -1))
